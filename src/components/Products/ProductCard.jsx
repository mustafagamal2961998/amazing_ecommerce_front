...
const ProductCard = (props) => {
  ...
  return (
    <div className='product relative w-full flex flex-col justify-center gap-5 rounded-md shadow'>
        <Image src={favImg} onMouseOver={() => setFavImg(redFav)} onMouseOut={() => setFavImg(fav)} width={20} height={20} className='absolute left-[20px] top-[15px] cursor-pointer w-[20px] h-[20px] object-cover' alt={props.title} />
        <Link href={`/products/${props.id}`}>
            <Image src={props.thumbnail} width={240} height={270} className='thumbnail w-full h-[200px] rounded-md object-cover' alt={props.title} />
        </Link>

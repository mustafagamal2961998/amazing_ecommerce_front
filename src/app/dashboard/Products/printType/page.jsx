  ...
  return (
    <div className='w-full flex flex-col items-center gap-6 p-5'>
      ...
      <div className={`w-full p-5 rounded-md ${deleteMood ? 'flex' : 'hidden'} flex-col justify-center items-center bg-[#D9D9D97D]`}>
       ...
       {/* <div className='w-full flex flex-col items-center gap-6 border-2 border-[#8080808C] rounded-lg p-3'>
        {
            printTypes.map((printType) => (
                <div key={printType.id} className='w-full flex justify-center items-center gap-6 border-b-2 border-[#8080808C] rounded-lg p-3'>
                    <p className='text-base font-bold'>{printType.name_ar}</p>
                    <p className='text-base font-bold'>{printType.name_en}</p>
                    <p className='text-base font-bold'>{printType.price} ر.س</p>
                    <p className={`text-base font-bold ${printType.status === 'active' ? 'text-green-500' : 'text-red-500'}`}>{printType.status === 'active' ? 'نشط' : 'غير نشط'}</p>
                    <FontAwesomeIcon
